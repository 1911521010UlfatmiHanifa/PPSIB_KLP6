<?php
    ...
    if(isset($_POST['tolakan'])){
        ...
        $status_peminjaman = "Ditolak Divisi Rumah Tangga";
        ...
        if($conn->affected_rows > 0){
            $pesan_sukses= "Data status peminjaman berhasil disimpan!";
        }
        else{
            $pesan_gagal= "Data status peminjaman gagal disimpan!";
        }
        $stmt->close();
    ...
    if(isset($_POST['setuju'])){
        ...
        $status_peminjaman = "Disetujui Divisi Rumah Tangga";
        $stmt=$conn->prepare("UPDATE peminjaman set status_peminjaman=? where id_peminjaman=?");
        $stmt->bind_param('si', $status_peminjaman, $id_peminjaman);
        $stmt->execute();
        ...
        <div class="row">
            ...
            <div class="card">
                ...
                <hr>
                    ...
                    <?php if($status_peminjaman == "Diajukan"){?>
                    <div class="col-12 text-center">
                        <form method="POST" action="">
                            <button class="btn btn-primary w-30" type="submit" name="setuju" onclick="return confirm('Anda yakin akan memenyetujui peminjaman inventaris ini?')">Setuju</button>
                            <button class="btn btn-danger w-30" type="submit" name="tolakan" onclick="return confirm('Anda yakin akan menolak peminjaman inventaris ini?')">Tolak</button>
                            <a class="btn btn-warning w-30" href="peminjaman_pengajuan_daftar.php">Back</a>
                        </form> 
                    </div>
                    <?php } else if($status_peminjaman == "Disetujui Divisi Rumah Tangga" || $status_peminjaman == "Ditolak Divisi Rumah Tangga"){?>
                    <div class="col-12">
