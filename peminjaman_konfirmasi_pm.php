<?php
    ...
    if(isset($_POST['tolakan'])){
        ...
        $status_harga1 = "Ditolak Divisi Pemasaran";
        $stmt=$conn->prepare("UPDATE peminjaman set status_harga=? where id_peminjaman=?");
        $stmt->bind_param('si', $status_harga1, $id_peminjaman);
        $stmt->execute();
    
        if($conn->affected_rows > 0){
            $pesan_sukses= "Data status ketersediaan peminjaman berhasil disimpan!";
        }
        else{
            $pesan_gagal= "Data status ketersediaan peminjaman gagal disimpan!";
        }
        $stmt->close();
    ...
    if(isset($_POST['setuju'])){
        ...
        $status_harga = "Disetujui Divisi Pemasaran";
        $stmt=$conn->prepare("UPDATE peminjaman set status_harga=? where id_peminjaman=?");
        $stmt->bind_param('si', $status_harga, $id_peminjaman);
        $stmt->execute();
        ...
        if($conn->affected_rows > 0){
            ...
            $pesan_gagal= "Data status ketersediaan peminjaman disimpan!";
        }
        $stmt->close();
    ...
    while($data = $hasil->fetch_assoc()){
        ...
        $status_harga2 =  $data['status_harga'];
        ...
        <div class="row">
            ...
            <div class="card">
                ...
                <hr>
                    ...
                    <?php if($status_harga2 == "Disetujui Divisi Pemasaran" || $status_harga2 == "Ditolak Divisi Pemasaran") {?>
                    <div class="col-12">
                        <a class="btn btn-warning w-100" href="konfirmasi_persetujuan.php">Back</a>
                    </div>
                    <?php } else if($status_peminjaman == "Disetujui Divisi Rumah Tangga"){?>
                    <div class="col-12 text-center">
                        <form method="POST" action="">
                            <button class="btn btn-primary w-30" type="submit" name="setuju" onclick="return confirm('Anda yakin akan memenyetujui ketersediaan peminjaman inventaris ini?')">Tersedia</button>
                            <button class="btn btn-danger w-30" type="submit" name="tolakan" onclick="return confirm('Anda yakin akan menolak ketersediaan peminjaman inventaris ini?')">Tidak Tersedia</button>
                            <a class="btn btn-warning w-30" href="konfirmasi_persetujuan.php">Back</a>
                        </form> 
                    </div>
                    <?php } ?>
                </div>
